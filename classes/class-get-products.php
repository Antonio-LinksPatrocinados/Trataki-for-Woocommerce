	...
	public function woosea_create_xml_feed ( $products, $feed_config, $header ) {
		...
		if ($feed_config['taxonomy'] == 'google_shopping'){
			...
			if ( ($header == "true") AND ($feed_config['nr_products_processed'] == 0) ) {
				...
				$xml->channel->addChild('description', 'Trataki for Woocommerce');
				$xml->asXML($file);	
			} else {
...
//					$xml->addAttribute('xmlns:atom', 'http://www.w3.org/2005/Atom');
					...
					$xml->addChild('title', htmlspecialchars($feed_config['projectname']));
					$xml->addChild('link', site_url());
					$xml->addChild('description', 'Trataki for Woocommerce');
					$xml->asXML($file);
				}
			} else {
				...
				if ($aantal > 0){
					...
					foreach ($products as $key => $value){
						...
						if ((is_array ( $value )) and (!empty( $value ))) {
							...
							$produto = wc_get_product( $value['id'] );
							...
							if ($produto->is_type( 'variable' )) {
								...
								$variacoes = $produto->get_available_variations();
								
								$variavel_group = $product->addChild('variants');
								
								foreach ($variacoes as $variacao_chave => $variacao) {
									
									$variavel_individual = $variavel_group->addChild('variant');
									
									foreach ($variacao as $chave => $valor){
										

										if ($chave == 'price_html') {
											// Nao colocar por causa de um bug
										}
										else {
											
											if ( is_array($valor) ) {
												
												$variavel_atributo = $variavel_individual->addChild($chave);
												
												foreach($valor as $chave2 => $valor2){
													$variavel_atributo->addChild($chave2,$valor2);
												}
	...
	public function woosea_get_products ( $project_config ) {
		...
		if(isset($project_config['product_variations'])){
			$post_type = array('product'); // Antonio -> $post_type = array('product', 'product_variation');
		} else {
		...
		$shipping_zones = $this->woosea_get_shipping_zones();
			...
			*/
			 ...
			 */
			if ( array_key_exists('mappings', $project_config) ) {
