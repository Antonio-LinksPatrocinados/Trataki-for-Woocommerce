 ...
 * Author:      Trataki
 * Plugin URI:  https://wwww.trataki.com
 * Author URI:  https://www.trataki.com
 * Developer:   Pedro Costa e José Ribeiro
 * License:     GPL3
 ...
 * WC tested up to: 5.1
 *
 * Trataki for WooCommerce is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * any later version.
 *
 * Trataki for WooCommerce is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with Trataki for WooCommerce. If not, see <http://www.gnu.org/licenses/>.
 */
 ...
 */
define( 'WOOCOMMERCESEA_PLUGIN_VERSION', '9.9.4' );
define( 'WOOCOMMERCESEA_PLUGIN_NAME', 'trataki-for-woocommerce' );
define( 'WOOCOMMERCESEA_PLUGIN_NAME_SHORT', 'trataki-for-woocommerce' );
...
add_action( 'init', 'actualizar_init' );
function actualizar_init() {
		if (!class_exists("Updater")) {
			require_once( 'classes/Updater.php' );
		}
		if ( is_admin() ) {
			new Updater( __FILE__, 'Antonio-LinksPatrocinados', "trataki-for-woocommerce" );
		}

}


//Para lead-time funcionar correctamente
function lead_time_fix() {

    $global_lead_time = get_option( 'wclt_global_time' );
    ...
    foreach ( $produtos as $produto ) {
        update_post_meta($produto->ID,'_wclt_lead_time',$global_lead_time);
    }
}

add_action('woocommerce_update_options_products_lead-time','lead_time_fix',999);
...
function woosea_scripts($hook) {
	...
	// Only register and enqueue JS scripts from within the plugin itself
     	if (preg_match("/trataki-for-woocommerce/i",$hook)){
...
function woosea_add_facebook_pixel( $product = null ){
	...
	if($add_facebook_pixel == "yes"){	
		...
		?>
		<!-- Facebook Pixel Code - Trataki for WooCommerce by Trataki -->
		<!------------------------------------------------------------------------------
...
function woosea_request_review(){
	...
	if(!empty( $cron_projects )){
		...
		if(($nr_projects > 0) AND ($is_active > $show_after) AND ($notification_interaction != "yes")){
			...
			echo '<table><tr><td></td><td><font color="green" style="font-weight:normal";><p>Hey, I noticed you have been using our plugin, <u>Trataki for WooCommerce by Trataki</u>, for over a week now and have created product feed projects with it - that\'s awesome! Could you please do Eva and me a BIG favor and give it a 5-star rating on WordPress? Just to help us spread the word and boost our motivation. We would greatly appreciate if you would do so :)<br/>~ Adtribes.io support team<br><ul><li><span class="ui-icon ui-icon-caret-1-e" style="display: inline-block;"></span><a href="https://wordpress.org/support/plugin/woo-product-feed-pro/reviews?rate=5#new-post" target="_blank" class="dismiss-review-notification">Ok, you deserve it</a></li><li><span class="ui-icon ui-icon-caret-1-e" style="display: inline-block;"></span><a href="#" class="dismiss-review-notification">Nope, maybe later</a></li><li><span class="ui-icon ui-icon-caret-1-e" style="display: inline-block;"></span><a href="#" class="dismiss-review-notification">I already did</a></li></ul></p></font></td></tr></table>';
			echo '</div>';	
		}
	}
}
// add_action('admin_notices', 'woosea_request_review'); Antonio
...
function woosea_hook_header() {
	$marker = sprintf('<!-- This website runs the Trataki for WooCommerce by Trataki plugin -->');
	echo "\n${marker}\n";
}
...
function woosea_menu_addition(){
		add_menu_page(__( 'Trataki for WooCommerce', 'woo-product-feed-pro' ), __( 'Trataki','woo-product-feed-pro' ), 'manage_options', __FILE__, 'woosea_generate_pages', esc_url( WOOCOMMERCESEA_PLUGIN_URL . '/images/icon-16x16.png'),99);
...
function woosea_fieldmapping_dropdown(){
	...
	foreach($attributes as $key => $value){
		...
		//$field_options .= "<optgroup label='$key'><strong>$key</strong>"; Antonio
...
function woosea_blog_widgets() {
	...
	add_meta_box('woosea_rss_dashboard_widget', __('Latest Trataki Tutorials', 'rc_mdm'), 'woosea_my_rss_box','dashboard','side','high');
}
